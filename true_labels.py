...
print(df_ThT_norm.columns[df_ThT_norm.loc['mean'] > 1.5])
# color these columns red
df_ThT_norm.loc['mean'].plot(kind='bar',color=['red' if x > 1.5 else 'blue' for x in df_ThT_norm.loc['mean']])
...
# print the columns that have a normalized mean more than 1.5
print(df_pFTAA_norm.columns[df_pFTAA_norm.loc['mean'] > 1.5])

# color these columns red
df_pFTAA_norm.loc['mean'].plot(kind='bar',color=['red' if x > 1.5 else 'blue' for x in df_pFTAA_norm.loc['mean']])

# %%
# print how many have a normalized mean more than 1.5 for ThT and pFTAA
print('ThT:',sum(df_ThT_norm.loc['mean'] > 1.5))
print('pFTAA:',sum(df_pFTAA_norm.loc['mean'] > 1.5))

# find the set that have a normalized mean more than 1.5 for either ThT or pFTAA
true_agg = set(df_ThT_norm.columns[df_ThT_norm.loc['mean'] > 1.5]) | set(df_pFTAA_norm.columns[df_pFTAA_norm.loc['mean'] > 1.5])
...
df_true_labels['ThT'] = df_ThT_norm.loc['mean'] > 1.5
df_true_labels['pFTAA'] = df_pFTAA_norm.loc['mean'] > 1.5
